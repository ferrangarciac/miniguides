	...
	for(i=0; i<language.LOCATION.length; i++){
		...
		for(z=0; z<LOCATION[i].entry_order.length; z++){
			switch(LOCATION[i].entry_order[z]){
				...
				case 2:	//STEAL ITEM
					createEntry(i, itemStealCount, entryCount, 2, itemGlobalDiv);
					entryCount++;
					itemStealCount++;
					break;
					
				case 3:	//DROP ITEM
					createEntry(i, itemDropCount, entryCount, 3, itemGlobalDiv);
					entryCount++;
					itemDropCount++;
					break;
					
				case 4:	//MONEY FIELD
					createEntry(i, moneyFieldCount, entryCount, 4, itemGlobalDiv);
					entryCount++;
					moneyFieldCount++;
					break;
					
				case 5:	//MONEY NPC
					createEntry(i, moneyNPCCount, entryCount, 5, itemGlobalDiv);
					entryCount++;
					moneyNPCCount++;
					break;
					
				case 6:	//ACHIEVEMENTS
					createEntry(i, achivementCount, entryCount, 6, itemGlobalDiv);
					entryCount++;
					achivementCount++;
					break;
					
				case 7:	//ADVICES
					createEntry(i, advicesCount, entryCount, 7, itemGlobalDiv);
					entryCount++;
					advicesCount++;
					break;
			}
	...
	switch (typeEntry){
		...
		case 1:
			...
			case 7:
				...
				var typeName = document.createElement('span');
				typeName.className = "type-name";
				typeName.innerHTML = language.MENU[7];
 	  ...
 	  if (showItems) {
        for (i = 0; i < location.map_connections.length; i++) {
          if (mapNumber == location.map_connections[i][1]) {
			...
			ctx.fillText(location.map_connections[i][0], location.map_connections[i][2] - (connections.width/2) +6, location.map_connections[i][3] - (connections.height/2) +12);
          }
        }
      }
