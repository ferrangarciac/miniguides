...
var tr = table[target].getElementsByTagName('tr');
for(i=1; i<tr.length-1;i++){
	var td = tr[i].getElementsByTagName('td');
	var a = td[1].getElementsByTagName('a');
	if(td[3].innerHTML.includes('Pokémon Ruby and Sapphire Versions')){
		if(td[2].innerHTML.includes('(hidden)')==false){
			items[count] = a[0].firstChild.data;		
			count++;
		}
	}
}
...
for(i=0;i<items.length;i++){
	for(x=0;x<ITEM3GEN.length;x++)
	if(ITEM3GEN[x] == items[i]){
		//console.log(i.toString());
		if(resultItems == ""){
			resultItems = resultItems + x;
			//resultItems = resultItems + items[i];
		}else{
			resultItems = resultItems + ", " + x;
			//resultItems = resultItems + ", " + items[i];
		}		
...
for(i=0;i<items.length;i++){
	...
	if(ITEM3GEN[x] == items[i]){
		//console.log(i.toString());
		if(resultHidden == ""){
			resultHidden = resultHidden + x;
			//resultHidden = resultHidden + items[i];
		}else{
			resultHidden = resultHidden + ", " + x;
			//resultHidden = resultHidden + ", " + items[i];
		}		
...
var result = ",\n{\nId: xx, //" + document.getElementById('firstHeading').innerHTML + "\nItems: [" + resultItems + "],\nHidden: [" + resultHidden + "],\nPokemon: []\n}";
