...
function createEntry(locationId, itemPos, entryCount, typeEntry, parentDiv){ //typeEntry: 0=no icon, 1=group icon, 2=all icon, 3=npc, 4=steal
	...
	switch (typeEntry){
		...
		case 0:
			...
			switch (item_icon_type){
				case 0:
					...
					text = language.ITEM[LOCATION[locationId].item_field[itemPos][0]];
					break;
				case 1:
					itemImage.setAttribute("src", './img/items/groupicons/' + GROUPICONS[LOCATION[locationId].item_field[itemPos][0]] + '.png');
					text = (itemPos + 1) + " - " + language.ITEM[LOCATION[locationId].item_field[itemPos][0]];
					break;
				case 2:
					itemImage.setAttribute("src", './img/items/noicons/noicon.png');
					text = (itemPos + 1) + " - " + language.ITEM[LOCATION[locationId].item_field[itemPos][0]];
			...
			if(LOCATION[locationId].item_field[itemPos][5]!==undefined){
				text = text  + " " + LOCATION[locationId].item_field[itemPos][5]
			}
		...
		case 1:
			...
			var itemName = document.createElement('span');
			text = language.ITEM[LOCATION[locationId].item_npc[itemPos][0]];
			
			if(LOCATION[locationId].item_npc[itemPos][6]!==undefined){
				text = text + " " + LOCATION[locationId].item_npc[itemPos][6];
			}
			itemName.innerHTML = text;
			...
			if(LOCATION[locationId].item_npc[itemPos][5] == true){
				var missable = document.createElement("div");
				missable.className = "missable-container";
      ...
      for (i = 0; i < location.map_connections.length; i++) {
        if (mapNumber == location.map_connections[i][1]) {
          ctx.drawImage(connections, location.map_connections[i][2] - (connections.width/2), location.map_connections[i][3] - (connections.height/2));
			...
			ctx.textAlign = "center"; 
			ctx.fillStyle = map_connection_text_stroke_color;
			
			var text;
			if(parseString(location.map_connections[i][0]).type=="/loc/:"){
				var locationId = parseString(location.map_connections[i][0]).number;
				text = language.LOCATION[locationId];
			}else if(parseString(location.map_connections[i][0]).type=="/com/:"){
				var commonId = parseString(location.map_connections[i][0]).number;
				text = language.COMMON[commonId];
			}else{
				text=location.map_connections[i][0];
			}
			
			ctx.fillText(text, location.map_connections[i][2]-1, location.map_connections[i][3]-1);//
			ctx.fillText(text, location.map_connections[i][2]+1, location.map_connections[i][3]-1);//
			ctx.fillText(text, location.map_connections[i][2]-1, location.map_connections[i][3]);//
			ctx.fillText(text, location.map_connections[i][2]+1, location.map_connections[i][3]);//
			ctx.fillText(text, location.map_connections[i][2]-1, location.map_connections[i][3]+1);//
			ctx.fillText(text, location.map_connections[i][2]+1, location.map_connections[i][3]+1);//
			ctx.fillStyle = map_connection_text_color;
			ctx.fillText(text, location.map_connections[i][2], location.map_connections[i][3]);
